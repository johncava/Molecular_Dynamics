...
class HNN(torch.nn.Module):
    ...
    def time_derivative(self, x, t=None, separate_fields=False):
        ...
        conservative_field = torch.zeros_like(x).cuda() # start out with both components set to 0
        solenoidal_field = torch.zeros_like(x).cuda()

        if self.field_type != 'solenoidal':
            dF1 = torch.autograd.grad(F1.sum(), x, create_graph=True)[0] # gradients for conservative field
            conservative_field = dF1 @ torch.eye(*self.M.shape).cuda()
