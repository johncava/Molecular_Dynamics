...
class Forces:
    ...
    def compute(self, pos, box, forces, returnDetails=False, explicit_forces=True):
        ...
        '''
        #print('Pos requires grad: ' + str(pos.requires_grad))
        ...
        for i in range(nsystems):
            spos = pos[i]
            #print('spos requires grad: ' + str(spos.requires_grad))
            sbox = box[i][torch.eye(3).bool()]#.requires_grad_(True)  # Use only the diagonal
            #print('sbox requires grad: ' + str(sbox.requires_grad))
            # Bonded terms
        ...
        '''
        if returnDetails:
            return [{k: v for k, v in pp.items()} for pp in pot]
        else:
