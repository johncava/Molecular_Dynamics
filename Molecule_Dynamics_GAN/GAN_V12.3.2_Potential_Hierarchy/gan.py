...
class Generator(nn.Module):
    ...
    def __init__(self):
        ...
        self.mlp3 = torch.nn.Conv1d(6,10,1)
        self.mlp4 = torch.nn.Linear(400,120)
    ...
    def forward(self,batch_size, max_steps):
        ...
        z = torch.cat([z,x],-1)
        z = torch.sigmoid(self.mlp3(z.view(batch_size,6,40))).view(batch_size,-1)
        z = self.mlp4(z)
        return t, z, picked_t
    ...
    def generation_step(self, t, max_steps):
        ...
        x = torch.stack(x,1).squeeze(-2)
        z = torch.cat([z,x],-1)
        z = torch.sigmoid(self.mlp3(z.view(1,6,40))).view(1,-1)
        z = self.mlp4(z)
        return z
##
