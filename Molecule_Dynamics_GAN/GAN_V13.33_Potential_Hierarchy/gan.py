##
# GAN_V13.3: Use the distance matrix as an input to the discriminator
##
import matplotlib
matplotlib.use('Agg')
...
for file_ in files:
    ...
    for frame_num in range(X.shape[0]):
        ...
        end_to_end_distance[frame_num].append(torch.cdist(x,x))
        del x
    
for i in range(len(end_to_end_distance)):
...
for batch in range(int(len(dataset)/batch_size)):
    ...
    for item in dataset[batch*batch_size:batch*batch_size + batch_size]:
        ...
        data = torch.cdist(data, data)
        batched_dist.append(data)
        batched_frame.append(fnum)

    batched_dist = torch.stack(batched_dist)
    batched_frame = torch.tensor(batched_frame).view(batch_size,1) / 1002
    new_dataset.append((batched_frame, batched_dist))
...
potential_keys = [['bonds','angles','dihedrals'] for _ in range(max_epochs)]
potential_factors = [(0.6,0.2,0.1)] + [(0.1,0.1,0.1) for _ in range(max_epochs)]
...
for epoch in range(max_epochs):
    ...
    for data in training_dataset:
        ...
        output = output.view(batch_size,40,3)
        ...
        for _ in range(Nd):
            ...
            label = torch.ones((batch_size,1)).float().cuda()
            d0 = data[0].float().cuda()
            d1 = data[1].float().cuda()
            pred = discriminator(d0,d1).squeeze(0)
