##
# GAN_V11: Same as GAN_V10 w/ target end to end distance of atom 1 and atom 10, averaged for each frame from the 200 trajectory dataset
# mse loss on dis_target and optimizing potential energy are done alternatively 
##
import matplotlib
matplotlib.use('Agg')
...
potential_keys = [['bonds','angles','dihedrals'], ['bonds','angles','dihedrals'], ['bonds','angles','dihedrals']]
potential_factors = [(0.6,0.2,0.1),(0.4, 0.4, 0.1),(0.2,0.2,0.5)]
...
for epoch in range(max_epochs):
    ...
    training_loss = []
    for iteration, data in enumerate(training_dataset):
        ###
        ...
        dis_factor = torch.tensor(100).float().cuda()
        ...
        if iteration % 2 == 0:
            ...
            for _ in range(2):
                ...
                dis = (output[0,:] - output[-1,:]).pow(2).sum().sqrt()
                target_dis = end_to_end_distance[t[0]]
                target_dis = torch.tensor(target_dis).cuda()
                dist_loss = dis_factor*F.mse_loss(dis, target_dis)
                dist_loss.backward()
                g_optimizer.step()
