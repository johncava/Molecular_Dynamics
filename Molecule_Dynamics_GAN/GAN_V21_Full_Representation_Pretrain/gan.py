...
for epoch in range(max_epochs):
    ...
    for data in training_dataset:
        ...
        t = t.float().cuda()
        z = q.rsample()
        # Capture z before the concatenation to t for KL divergence
...
for epoch in range(max_epochs):
    ...
    for data in training_dataset:
        ...
        ###
        bonds_factor = torch.tensor(0.1).float().cuda()
        angle_factor = torch.tensor(0.1).float().cuda()
        dihedral_factor = torch.tensor(0.1).float().cuda()
        improper_factor = torch.tensor(0.1).float().cuda()
        lj_factor = torch.tensor(0.1).float().cuda()
        electrostatics_factor = torch.tensor(0.1).float().cuda()
        repulsion_factor = torch.tensor(0.1).float().cuda()
