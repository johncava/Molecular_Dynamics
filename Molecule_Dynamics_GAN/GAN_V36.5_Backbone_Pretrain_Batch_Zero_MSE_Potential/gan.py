##
# GAN_V34.8: just like GAN_V34.7 but with batch size of 16 vs. 32
##
import matplotlib
matplotlib.use('Agg')
...
min_val = 1e9
for q, learning_rate in enumerate([1e-2, 1e-2, 1e-2, 1e-2, 1e-3, 1e-3]):
    print('=====')
    print(q)    
    ...
    for epoch in range(max_epochs):
        ...
        for i, data in enumerate(pretrain_dataloader):
            ...
            x = x.view(bsize, 40*3)
            target_phi = data['phi']
            target_bonds = data['bonds']
            target_angles = data['angles']
            target_dihedrals = data['dihedrals']
            target_impropers = data['impropers']
            ...
            total_bonds = torch.zeros(1,1).cuda()
            total_dihedrals = torch.zeros(1,1).cuda()
            total_angles = torch.zeros(1,1).cuda()
            total_impropers = torch.zeros(1,1).cuda()
            total_phi = torch.zeros(1,1).cuda()

            for px, t_phi, t_bonds, t_angles, t_dihedrals, t_impropers, pt in zip(pred_x,target_phi,target_bonds,target_angles,target_dihedrals, target_impropers, t):
                potential, phi = sys_decal.calc_energy(px.view(40,3,1),pt)
                #total_phi +=  F.mse_loss(phi, t_phi)
                for key in potential[0].keys():
                    if key == 'bonds':
                        total_bonds += F.mse_loss(bonds_factor*potential[0][key],t_bonds)
                    if key == 'angles':
                        total_angles += F.mse_loss(angle_factor*potential[0][key], t_angles)
                    if key == 'dihedrals':
                        total_dihedrals += F.mse_loss(dihedral_factor*potential[0][key], t_dihedrals)
                    if key == 'impropers':
                        total_impropers += F.mse_loss(improper_factor*potential[0][key], t_impropers)
            
            total_potential = total_bonds/batch_size 
            if q > 1:
                total_potential +=  total_angles/batch_size 
            if q > 2:
                total_potential += total_dihedrals/batch_size 
            if q > 3:
                total_potential += total_impropers/batch_size
            #recon_loss = F.mse_loss(pred_x,x) + (total_phi/batch_size) + total_potential
            #total_loss = (one - alpha)*total_pot + alpha*recon_loss
            #total_loss = total_pot + recon_loss
            total_loss = total_potential
            total_loss.backward()
