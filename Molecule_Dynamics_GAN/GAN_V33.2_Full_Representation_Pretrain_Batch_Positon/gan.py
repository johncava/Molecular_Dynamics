##
# GAN_V32: Full Representation Pretrain Batch; switch alpha and (1 - alpha)
##
import matplotlib
matplotlib.use('Agg')
...
class PotentialEnergyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        potential, phi = self.sys_decal.calc_energy(frame.view(104,3,1),frame_num)
...
for epoch in range(max_epochs):
    ...
    for i, data in enumerate(pretrain_dataloader):
        ...
        target_phi = data['phi']
        z = torch.normal(0,1,size=(bsize,31)).cuda()
        t = t.unsqueeze(-1).float().cuda()/max_steps
        z = torch.cat((t,z),1)

        pred_x = decoder(z)
        pred_dist = torch.cdist(pred_x.view(bsize, 104,3),pred_x.view(bsize, 104,3))
        target_dist = torch.cdist(x.view(bsize, 104,3), x.view(bsize, 104, 3))

        pretrain_optimizer.zero_grad()
        total_pot = torch.zeros(1,1).cuda()
        total_phi = torch.zeros(1,1).cuda()

        recon_loss = F.mse_loss(pred_dist, target_dist)

        for px, t_phi, pt in zip(pred_x,target_phi, t):
            potential, phi = sys_decal.calc_energy(px.view(104,3,1),pt)
