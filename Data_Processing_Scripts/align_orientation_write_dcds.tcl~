...
proc randomize_orientation_dcd { psffile dcdfile pdbOutpuBaseName } { 
    ...
    $selall frame 0
    moveCOMtoOrigin $selall
    #set transMatrix [randomize_orientation_sel $selall]
    
    set fp [open $pdbOutpuBaseName w+]
   
    ### COMMENTED OUT BECAUSE I DONT WANT TO SHIFT ANYTHING HERE. 
    #for {set i 0} {$i < $nf} {incr i} {
    #$selall frame $i
    #moveCOMtoOrigin $selall
    #$selall move $transMatrix

    set backbonesel [atomselect $myMolID backbone]
    $backbonesel frame $i
    set datxyz [$backbonesel get {x y z}]
    puts $fp $datxyz
    #puts $fp "\n"
    }
    close $fp
    ...
    # for {set i 0} {$i < $nf} {incr i} {
    # $selall frame $i
    # set randomize_orientation_sel $selall $
...
for {set k 0} {$k < 1} {incr k} {
    ...
    puts $replica
    randomize_orientation_dcd $replica/my.pdb $replica/smd_out.dcd ./xyzCOM/rep$replica-$k.prexyz 
