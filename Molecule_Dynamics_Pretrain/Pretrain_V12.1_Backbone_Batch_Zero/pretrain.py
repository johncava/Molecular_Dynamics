...
min_val = 1e9
for q, learning_rate in enumerate([1e-2, 1e-2,1e-2, 1e-3, 1e-3, 1e-3,1e-3]):
    ...
    for epoch in range(max_epochs):
        ...
        for i, data in enumerate(pretrain_dataloader):
            ...
            dis_factor = torch.tensor(100.0).float().cuda()
            ...
            dist_pred = torch.cdist(pred_x,pred_x)
            dist_traget = torrch.cdist(x,x)
            recon_loss = total_phi/batch_size + dis_factor*F.mse_loss(dist_pred, dist_target)
            #total_loss = alpha*total_pot + (one - alpha)*recon_loss
            if q < 2:
                total_loss = total_pot
            else:
