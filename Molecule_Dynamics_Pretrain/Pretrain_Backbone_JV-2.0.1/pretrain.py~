...
for q, learning_rate in enumerate([1e-2, 1e-2, 1e-2, 1e-3, 1e-3, 1e-3, 1e-3, 1e-4]):
    ...
    for epoch in range(max_epochs):
        ...
        for i, data in enumerate(pretrain_dataloader):
            ...
            bonds_factor = torch.tensor(1.0).float().cuda() * alpha 
            angle_factor = torch.tensor(1.0).float().cuda() * alpha * alpha
            dihedral_factor = torch.tensor(1.0).float().cuda() * alpha * alpha * alpha
            improper_factor = torch.tensor(1.0).float().cuda() * alpha * alpha * alpha
            lj_factor = torch.tensor(1.0).float().cuda()
            electrostatics_factor = torch.tensor(1.0).float().cuda() * alpha * alpha * alpha * alpha
            ...
            total_loss = alpha*total_pot + (one - alpha)*total_phi
            total_loss.backward()
        ...
        if l < min_val:
            ...
            print("Mean epoch_pretrain_loss", l)
