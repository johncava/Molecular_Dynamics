...
# Usage:
##############################################################
# Notes: cGAN_D_pos-2.3.0 combines label noise and label smoothing
##############################################################
'''
###
# Import
...
def update_D(data, discriminator, decoder, noise_rate=0.20, alpha=0.9, beta=0.3):
    '''
    # (1) Update D Network: maximize log(D(x)) + log (1 - D(G(z)))
    '''
    true_label_smooth = torch.tensor(alpha).float().cuda()
    ...
    # D(G(z)))
    pred = discriminator(t,output).squeeze(0)
    fake_label_smooth = torch.tensor(beta).float().cuda()
...
noise_rates = [0.05]*max_epochs_GAN
smoothings = [(0.9,0.2)]*max_epochs_GAN
for epoch in range(max_epochs_GAN):
    ...
    for i, data in enumerate(training_dataloader):
        ...
        for _ in range(Nd):
            print("update_D")
            update_D(data, discriminator, decoder,noise_rate=noise_rates[epoch], alpha=smoothings[epoch][0], beta=smoothings[epoch][1])
