...
class SystemDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, psf_file, parameter_file, traj_files, colvar, pretrain=False, transform=None, stride=10):
        ...
        self.data = dataset
        self.num_frames = X_data.shape[0]
...
def update_pretrain(pretrain_optimizer, data, decoder, V_intro_epoch, dist_decline_epoch):
    ...
    print(f"alpha:{alpha}\nalpha2:{alpha2}\nPot:{total_pot}")
    print(f"Total pot is {total_pot.item()}")
...
for epoch in range(max_epochs_pre):
    ...
    for i, data in enumerate(pretrain_dataloader):
        bs = len(data[1])
        total_loss = update_pretrain(pretrain_optimizer, data, decoder, V_intro_epoch, dist_decline_epoch)
    ...
    print(f"Pre-Training is {(max_epochs_pre + epoch)/(max_epochs_pre*8)*100:.4f} % complete")
