...
for file_ in files:
    ...
    X = X_positions
    print(f"Number of frames in {file_}",len(X))
...
# Dataset size
print("Training Set size is",len(training_dataset))
...
class PotentialEnergyDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data, colvar, transform=None):
        ...
        self.sys_decal = Energy(data_dir, psf_file, parameter_file, colvar=colvar, device="cuda")
...
pretrain_loss = []
import time
print("Pretrain data loader size", len(pretrain_dataloader))
...
min_val = 1e9
# for q, learning_rate in enumerate([1e-2, 1e-3, 1e-4]):
q = 0
learning_rate = 1e-2
