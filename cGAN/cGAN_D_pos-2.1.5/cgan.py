...
def update_D(data, discriminator, decoder, smooth):
    '''
    # (1) Update D Network: maximize log(D(x)) + log (1 - D(G(z)))
    '''
    true_label_smooth = torch.tensor(smooth[0]).float().cuda()
    ...
    # D(G(z)))
    pred = discriminator(t,output).squeeze(0)
    fake_label_smooth = torch.tensor(smooth[1]).float().cuda()
...
smooth_params = [(0.9,0.3)]*20 + [(0.95, 0.2)]*5 + [(1.0, 0.0)]*5
for epoch in range(max_epochs_GAN):
    ...
    for i, data in enumerate(training_dataloader):
        ...
        for _ in range(Nd):
            print("update_D")
            param_pick = smooth_params[epoch]
            update_D(data, discriminator, decoder, param_pick)
