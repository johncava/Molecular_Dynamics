...
class Energy:
    ...
    def __init__(self, psf_file, parameter_file,
        ...
        if not colvar is None:
            ...
            self.colvar_cent_0 = torch.tensor(colvar['cent_0'], device=self.device, dtype=self.dtype,requires_grad=True)
            self.colvar_cent_1 = torch.tensor(colvar['cent_1'], device=self.device, dtype=self.dtype,requires_grad=True)
...
# GAN functions
###
def update_pretrain(pretrain_optimizer, data, decoder, V_intro_epoch, dist_decline_epoch):
    data, t = data
    if not epoch > V_intro_epoch:
        alpha = torch.tensor(0.0).float().cuda()
    else:
        alpha = torch.tensor((epoch-V_intro_epoch)/(max_epochs_pre-V_intro_epoch-1)).float().cuda()

    if not epoch > dist_decline_epoch:
        alpha2 = torch.tensor(0.0).float().cuda()
    else:
        alpha2 = torch.tensor((epoch-dist_decline_epoch)/(max_epochs_pre-dist_decline_epoch-1)).float().cuda()
    ...
    for px, t_phi, pt in zip(pred_x, target_phi, t):
        if count % int(batch_size/2) == 0:
            predictions.append(px.view(num_atoms,3))
        count+=1
        potential, phi = pred_sys.calc_energy(px.view(num_atoms,3,1), pt)
    ...
    dist_pred = torch.cdist(pred_x.view(bsize,num_atoms,3),pred_x.view(bsize,num_atoms,3))
    dist_target = torch.cdist(x.view(bsize,num_atoms,3),x.view(bsize,num_atoms,3))
...
def update_G(decoder, discriminator):
    ...
    # Generator
    t, output, pt = decoder.generate(bs,num_frames)
    output = output.view(bs,num_atoms*3)
    # D(G(z)))
    phis = []
    append = phis.append
    for out, p in zip(output,t):
        _, phi = pred_sys.calc_energy(out.view(num_atoms,3,1),p)
...
def update_D(data, discriminator, decoder):
    ...
    t_real = t_real.view(bs,1)
    pt = t_real/num_frames
    x = data['frame']
...
def update_G_net(decoder):
    ...
    for out, pt in zip(output,t):
        ...
        # print(pt,potential[0]['E2End Harm'])
        total_pot += torch.sum(torch.stack([potential[0][key] for key in potential[0].keys() if not key == 'E2End Harm']))
        total_pot += torch.tensor(200.0).float().cuda()*potential[0]['E2End Harm']
    total_pot = total_pot/bs
...
#     epoch_pretrain_loss = []
    ...
    #      update_pretrain(pretrain_optimizer, data, decoder, V_intro_epoch, dist_decline_epoch)
    ...
    # print(f"Pre-Training is {(q*max_epochs_pre + epoch)/(max_epochs_pre*8)*100:.4f} % complete")
