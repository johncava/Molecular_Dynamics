...
parser.add_argument('--data', type=str, default='/scratch/nichola2/singharoylab/10_deca_alanine/99/', help='directory for input data')
...
parser.add_argument('--hidden_size', type=int,default=24,help='hidden size')
...
parser.add_argument('--split',type=float,default=0.99,help='split percentage')
...
## this function takes in dataset and returns a variation with noise and calculated velocities and accelerations
def construct_data_w_noise(dataset, std):
...
#     std = 0.003
    ...
    for frame in range(len(dataset)):
        dat = dataset[frame]
        payload.append((dat[0] + np.random.normal(0, std, dat[0].shape), dat[1] + np.random.normal(0, std, dat[1].shape)))
    dataset = payload ## overriding the previous dataset
    ...
    for data in dataset:
        ...
        #print(y.shape)
        velocity_history = []
        ## most recent is to the left
        for index in range(history_size-1,0,-1): ## adding the history size 4 3 2 1 
            d_velocity = x[index,:,:] - x[index-1,:,:]
            velocity_history.append(d_velocity)
        v_i = velocity_history[0] ## because the data is being added in the order of 4 3 2 1, so the most "recent" velocity is at index 0
        v_f = y - x[-1,:,:]
...
class GATEncoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        return self.gat1(x=x, edge_index=edge_index).relu()
...
class GATDecoder(torch.nn.Module):
    ...
    def forward(self, x, edge_index):
        return self.gat1(x=x, edge_index=edge_index).tanh() ## make sure to change this if removing normalization
...
for epoch in range(max_epochs):
    ...
    ## reconstructing the new dataset every epoch
    
    new_dataset = construct_data_w_noise(dataset, std)
    ...
    for data in training_dataset:
        ...
        transform0 = T.KNNGraph(k=4)
        x = transform0(x)
        
        transform = T.Distance(norm=True)
        x = transform(x)
        x.edge_attr = x.edge_attr.pow(2) ## squaring
...
for data in copy_dataset[1:]:
    ...
    _,_,acceleration,y = data
    #print(x.shape,velocity_history.shape,acceleration.shape,y.shape)

    X_pos = x[::]
    input_history = np.concatenate((X_pos, velocity_history),axis=-1)
    x = torch.tensor(x).float().cuda()
    input_history = torch.tensor(input_history).float().cuda()
    v_i = torch.tensor(velocity_history[:,3:6]).float().cuda()
    x = Data(x=input_history,pos=x)
    transform0 = T.KNNGraph(k=4)
    x = transform0(x)
    transform = T.Distance(norm=True)
    x = transform(x)
    ...
    velocity_history = np.concatenate((v_pred.cpu().detach().numpy(), velocity_history),axis=-1)
    velocity_history = velocity_history[:,:-3]
    predictions.append(x_final)
    
    # Update the new x for autoregressive
    x = x_final.cpu().detach().numpy()
