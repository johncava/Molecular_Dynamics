...
with tf.Session() as sess:
    ...
    with open("Generation_round" + str(wround) + "_" + str(lead_time) + "_" + str(n_hidden) + "_" + str(maxSeqlength) + "_" + str(chunk) + ".txt", 'w') as outputfile2:
        with open("Generation_round" + str(wround) + "_unscaled.txt", 'w') as outputfile:
            ...
            pred_x_all = tf.convert_to_tensor(distribution[:,0] + (np.random.normal(0, 1, len(all_x)) * distribution[:,1] * lambd))
            pred_y_all=tf.convert_to_tensor(distribution[:,2] + (np.random.normal(0, 1, len(all_x)) * distribution[:,3] * lambd))
            pred_z_all=tf.convert_to_tensor(distribution[:,4] + (np.random.normal(0, 1, len(all_x)) * distribution[:,5] * lambd))
