...
gat_decoder = GATDecoder(2, output_size)
...
for epoch in range(max_epochs):
    ...
    for data in training_dataset:
        ...
        for index in range(lead_time):
            ...
            # Input the output of the LSTM into a GAT decoder
            output = output.view(number_of_particles,5)
            z = Data(x=output[:,3:],pos=output[:,:3])
            transform0 = T.KNNGraph(k=6)
            z = transform0(z)
            transform = T.Distance()
            z = transform(z)

            z_decoded = gat_decoder(z.x, z.edge_index)
            output = z_decoded.view(1,number_of_particles,5)
