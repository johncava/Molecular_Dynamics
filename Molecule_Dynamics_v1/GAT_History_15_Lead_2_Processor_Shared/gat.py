...
class GATProcessor(torch.nn.Module):
    ...
    def forward(self, x_i):
        for _ in range(self.M):
            ...
            x_i = transform(x_i)
            x_i = x_i.pos + self.processor(x_i.pos, x_i.edge_index).relu()
        return x_i
...
optimizer = optim.Adam(list(gat_encoder.parameters()) + list(gat_decoder.parameters()) +
                        list(gat_processor.parameters()), lr=learning_rate)
