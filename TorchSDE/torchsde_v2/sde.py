...
def main(
        ...
        steps=1,            # How many steps to train both generator and discriminator for.
    ...
    for step in trange:
        ...
        if step % 10 == 0:
            print('hi')
            potential_keys = ['bonds','angles','dihedrals']
            potential_factors = [(0.1,0.1,0.1)]
            p_factors = potential_factors[0]
            bonds_factor = torch.tensor(p_factors[0]).float().to(device)
            angle_factor = torch.tensor(p_factors[1]).float().to(device)
            dihedral_factor = torch.tensor(p_factors[2]).float().to(device)
            generator_optimiser.zero_grad()
            for i in range(2):
                ts_sample = random.choices(list(range(1002)),k=1)
                ts_sample = torch.tensor(ts_sample).to(device)
                generated_samples = generator(ts, 1)
                sample_frame = generated_samples[:,i,1:]
                sample_frame = sample_frame.view(40,3,1)
                sys_decal = Energy(data_dir, psf_file, parameter_file)
                potential = sys_decal.calc_energy(sample_frame)
                total_pot = torch.zeros(1,1).to(device)
    ...
    generated_samples = generator(ts,10)
