...
# atomistic forces
set Foutfile [open "./smd_out.forces.dat" w]
set Ffreq $dcdfreq
set atomList [list 4 14 24 34 44 54 64 74 84 99]
...
tclForcesScript {
  # The IDs of the four atoms defining the dihedral
    ...
    proc calcforces {} {
        global f atomList Foutfile Ffreq
        set frame [getstep]
        # Write frequencies
        if {int(fmod([expr $frame],$Ffreq))==0 && ([expr $frame-1]!=0)} {
        loadcoords p
        ...
        foreach atom $atomList {
            # puts "Coord: $p(14)"
            puts $Foutfile "Force: $frame $atom $f($atom)"
        }
        }
    }
}
